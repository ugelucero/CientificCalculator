...
/// // let result = evaluate("2+3*4", AngleMode::Rad)?;
/// // assert_eq!(result, 14.0);
/// ```
pub fn evaluate(input: &str, angle_mode: AngleMode) -> Result<f64, CalcError> {
    let tokens = tokenizer::tokenize(input)?;
    let rpn = ast::shunting_yard(&tokens)?;
    evaluator::evaluate_ast(&rpn, angle_mode)
}
...
mod tests {
    ...
    fn test_evaluate_simple() {
        let result = evaluate("2+3", AngleMode::Rad).unwrap();
        assert!((result - 5.0).abs() < 1e-10);
    }

    #[test]
    fn test_evaluate_with_whitespace() {
        let result = evaluate("  2  +  3 * 4 ", AngleMode::Rad).unwrap();
    ...
    fn test_evaluate_power_associativity() {
        // 2^3^2 = 512
        let result = evaluate("2^3^2", AngleMode::Rad).unwrap();
        assert!((result - 512.0).abs() < 1e-10);
    }

    #[test]
    fn test_evaluate_parse_error() {
        let result = evaluate("2+", AngleMode::Rad);
        assert!(result.is_err());
    }

    #[test]
    fn test_evaluate_domain_error() {
        let result = evaluate("sqrt(-1)", AngleMode::Rad);
        assert!(result.is_err());
    }

    #[test]
    fn test_evaluate_division_by_zero() {
        let result = evaluate("1/0", AngleMode::Rad);
        assert!(result.is_err());
    }
}
